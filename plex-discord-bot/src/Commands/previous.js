...
const logger = new Logger('PreviousCommand');

class PreviousCommand extends Command {
	constructor() {
		super({
			name: 'previous',
			description: '⏮️ Skip to previous Plex media via Firefox extension',
			type: 'SLASH',
			run: async (_, interaction) => {
				try {
					...
					logger.log('Sending previous command to Firefox');
					await wsClient.sendCommand({ action: 'previous' });

					return interaction.editReply({
						embeds: [
							new EmbedBuilder()
								.setColor(0x00CCFF)
								.setTitle('⏮️ Skipped to Previous')
								.setDescription('Skipped to previous media in your Plex playlist via Discord')
