...
class ResumeCommand extends Command {
	constructor() {
		super({
			name: 'resume',
			description: '▶️ Resume Plex playback via Firefox extension',
			type: 'SLASH',
			run: async (_, interaction) => {
				try {
					...
					// Send play command
					logger.log('Sending resume (play) command to Firefox');
					...
					return interaction.editReply({
						embeds: [
							new EmbedBuilder()
								...
								.setDescription('Plex playback has been resumed via Discord')
